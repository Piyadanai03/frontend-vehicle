...
export default function CameraReport() {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `https://n8n-vehicle.app.n8n.cloud/webhook/vehicle_count/all?type=camera&id=${cameraId}&start=${start}&stop=${stop}`
        );
        if (!response.ok) {
  ...
  const chartData = {
    ...
    datasets: [
      {
        label: "เข้า",
        data: ["car", "motorcycle", "bus"].map(
          (type) =>
            data.details.find(
              (d: any) =>
                d.vehicle_type_name === type && d.direction_type_name === "in"
            )?.count || 0
        ),
      ...
      },
      {
        ...
        data: ["car", "motorcycle", "bus"].map(
          (type) =>
            data.details.find(
              (d: any) =>
                d.vehicle_type_name === type && d.direction_type_name === "out"
            )?.count || 0
        ),
